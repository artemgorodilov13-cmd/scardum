...
def comment_to_dict(c):
    return {
        ...
        'game_title': c.game.title if c.game else '—',
        'name': c.name,
        'text': c.text,
...
def api_list_comments():
    ...
    approved = Comment.query.filter_by(is_approved=True).order_by(Comment.created_at.desc()).all()
    return jsonify({
        'pending': [comment_to_dict(c) for c in pending],
        'approved': [comment_to_dict(c) for c in approved],
    })


@app.route('/admin/api/comments/<int:comment_id>/approve', methods=['POST'])
@api_admin_required
...
@app.route('/admin/api/comments/<int:comment_id>', methods=['DELETE'])
@api_admin_required
...
@app.route('/admin/api/suggestions')
@api_admin_required
def api_list_suggestions():
    pending = Suggestion.query.filter_by(is_reviewed=False).order_by(Suggestion.created_at.desc()).all()
    reviewed = Suggestion.query.filter_by(is_reviewed=True).order_by(Suggestion.created_at.desc()).all()
    return jsonify({
        'pending': [suggestion_to_dict(s) for s in pending],
        'reviewed': [suggestion_to_dict(s) for s in reviewed],
    })


@app.route('/admin/api/suggestions/<int:suggestion_id>/mark-reviewed', methods=['POST'])
@api_admin_required
def api_mark_suggestion_reviewed(suggestion_id):
    suggestion = Suggestion.query.get_or_404(suggestion_id)
    suggestion.is_reviewed = True
    db.session.commit()
    return jsonify({'status': 'ok'})
...
@app.route('/admin/api/suggestions/<int:suggestion_id>', methods=['DELETE'])
@api_admin_required
def api_delete_suggestion(suggestion_id):
    suggestion = Suggestion.query.get_or_404(suggestion_id)
    db.session.delete(suggestion)
    db.session.commit()
    return jsonify({'status': 'deleted'})
